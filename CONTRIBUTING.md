...
Contributions must be sent to <~hadronized/glsl-devel@lists.sr.ht>.
...
```sh
git config --local sendemail.to "~hadronized/glsl-devel@lists.sr.ht"
```
...
```sh
git config --local format.subjectprefix "PATCH glsl"
```
